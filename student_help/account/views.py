...
""" import sys
sys.path.insert(1,'../authentification') """

""" from django.shortcuts import get_object_or_404
from Student_Help.Student_Help.resultergistration.models import Profile """
...
def profile(request , pk):
    ...
    return render(request, 'profile.html' , {'user_profile' : user_profile ,
                        'user' : follower ,    'user_object':user_object,   'user_followers': user_followers,
...
def index(request):
    ...
    unread_notifications_count = Notification.objects.exclude(user=request.user, is_read=False).count()
    ...
    for notification in notifications:
        user_profile = Profile.objects.get(user=notification.user)
    ...
    return render(request, 'index.html' , {'user_profile': user_profile , 'notification_details' : notification_details ,
